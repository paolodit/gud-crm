...
import { db } from "@/db";
import { auditEvents, companies, opportunities, stages } from "@/db/schema";
import { deliveryDetails, deliveryUpdateSchema, getLiveProjects } from "@/lib/domain/delivery";
...
export async function updateDelivery(actor: Actor, input: unknown) {
  ...
  if (actor.storageMode === "sqlite") {
    updateLocalBoardSnapshot((snapshot) => {
      ...
      saved = deliveryDetails({ ...deliveryDetails(project.delivery), ...data.delivery });
    ...
    await db.transaction(async (tx) => {
      ...
      saved = deliveryDetails({ ...deliveryDetails(record.opportunity.delivery), ...data.delivery });
