...
export function buildPostgresPromotionSql(snapshot: BoardSnapshot, input: {
  ...
  for (const company of companies.values()) {
    sql.push(`INSERT INTO companies (id, organisation_id, name, normalised_name, website_url, linkedin_url, scale_note, sector, fit_score, research_note, source_urls, do_not_contact, archived_at, import_metadata, created_at, updated_at)
      VALUES (${text(company.id)}::uuid, ${text(organisationId)}::uuid, ${text(company.name)}, ${text(normaliseName(company.name))}, ${nullableText(company.websiteUrl)}, ${nullableText(company.linkedinUrl)}, ${nullableText(company.scaleNote)}, ${nullableText(company.sector)}, ${nullableInteger(company.fitScore)}, ${nullableText(company.researchNote)}, ${json(company.sourceUrls ?? [])}, ${bool(company.doNotContact)}, ${nullableTimestamp(company.archivedAt)}, ${json({ promotedFrom: "sqlite" })}, now(), now())
      ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, normalised_name = EXCLUDED.normalised_name, website_url = EXCLUDED.website_url, linkedin_url = EXCLUDED.linkedin_url, scale_note = EXCLUDED.scale_note, sector = EXCLUDED.sector, fit_score = EXCLUDED.fit_score, research_note = EXCLUDED.research_note, source_urls = EXCLUDED.source_urls, do_not_contact = EXCLUDED.do_not_contact, archived_at = EXCLUDED.archived_at, import_metadata = EXCLUDED.import_metadata, updated_at = now();`);
  }
  ...
  for (const opportunity of snapshot.opportunities) {
    sql.push(`INSERT INTO opportunities (id, organisation_id, pipeline_id, offer_id, company_id, stage_id, position, owner_id, title, priority, temperature, value, probability, expected_close_date, outreach_angle, last_activity_at, next_action_at, no_next_action_reason, archived_at, import_metadata, created_at, updated_at)
      VALUES (${text(opportunity.id)}::uuid, ${text(organisationId)}::uuid, ${text(snapshot.pipeline.id)}::uuid, ${opportunity.offer ? `${text(opportunity.offer.id)}::uuid` : "NULL"}, ${text(opportunity.company.id)}::uuid, ${text(opportunity.stageId)}::uuid, ${integer(opportunity.position)}, ${ownerId(opportunity.owner, snapshot.users, administratorEmail)}, ${text(opportunity.title)}, ${text(opportunity.priority)}::priority, ${text(opportunity.temperature)}::temperature, ${nullableNumber(opportunity.expectedValue)}, ${nullableInteger(opportunity.probability)}, ${nullableTimestamp(opportunity.expectedCloseDate)}, ${nullableText(opportunity.outreachAngle)}, ${nullableTimestamp(opportunity.lastActivityAt)}, ${nullableTimestamp(opportunity.nextActionAt)}, ${nullableText(opportunity.noNextActionReason)}, ${nullableTimestamp(opportunity.archivedAt)}, ${json({ promotedFrom: "sqlite", demoExample: opportunity.isExample === true })}, now(), now())
      ON CONFLICT (id) DO UPDATE SET offer_id = EXCLUDED.offer_id, company_id = EXCLUDED.company_id, stage_id = EXCLUDED.stage_id, position = EXCLUDED.position, owner_id = EXCLUDED.owner_id, title = EXCLUDED.title, priority = EXCLUDED.priority, temperature = EXCLUDED.temperature, value = EXCLUDED.value, probability = EXCLUDED.probability, expected_close_date = EXCLUDED.expected_close_date, outreach_angle = EXCLUDED.outreach_angle, last_activity_at = EXCLUDED.last_activity_at, next_action_at = EXCLUDED.next_action_at, no_next_action_reason = EXCLUDED.no_next_action_reason, archived_at = EXCLUDED.archived_at, import_metadata = EXCLUDED.import_metadata, updated_at = now();`);

    if (opportunity.delivery) sql.push(`UPDATE opportunities SET delivery = ${json(opportunity.delivery)} WHERE id = ${text(opportunity.id)}::uuid AND organisation_id = ${text(organisationId)}::uuid;`);
