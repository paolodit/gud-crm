...
export function buildPostgresPromotionSql(snapshot: BoardSnapshot, input: {
  ...
  for (const opportunity of snapshot.opportunities) {
    sql.push(`INSERT INTO opportunities (id, organisation_id, pipeline_id, offer_id, company_id, stage_id, position, owner_id, title, priority, temperature, value, probability, expected_close_date, outreach_angle, last_activity_at, next_action_at, no_next_action_reason, import_metadata, created_at, updated_at)
      ...
      ON CONFLICT (id) DO UPDATE SET offer_id = EXCLUDED.offer_id, company_id = EXCLUDED.company_id, stage_id = EXCLUDED.stage_id, position = EXCLUDED.position, owner_id = EXCLUDED.owner_id, title = EXCLUDED.title, priority = EXCLUDED.priority, temperature = EXCLUDED.temperature, value = EXCLUDED.value, probability = EXCLUDED.probability, expected_close_date = EXCLUDED.expected_close_date, outreach_angle = EXCLUDED.outreach_angle, last_activity_at = EXCLUDED.last_activity_at, next_action_at = EXCLUDED.next_action_at, no_next_action_reason = EXCLUDED.no_next_action_reason, import_metadata = EXCLUDED.import_metadata, updated_at = now();`);
