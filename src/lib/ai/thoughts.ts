...
export function thoughtOutline(note: Thought): ExplorationDocument {
  return {
...
export async function generateThoughtExploration(note: Thought, research: boolean): Promise<ExplorationDocument> {
  ...
  const response = await client.responses.create({
    ...
    input: [
      ...
      { role: "user", content: JSON.stringify({ title: note.title, text: note.body, checklist: note.checklist.map(({ text, done }) => ({ text, done })) }) },
    ],
