...
async function selected(actor: CurrentMember, record: { kind: "lead" | "project"; id: string }) {
...
export async function openRecord(actor: CurrentMember, input: unknown) {
  ...
  return { ...record, href: recordHref(record), company: project?.companyName ?? lead!.company.name, title: project?.title ?? lead!.title, value: project?.delivery.projectValue ?? lead?.expectedValue ?? null,
    ...
    details: project ? { ownerId: project.ownerId, offerId: project.offerId, nextMilestone: project.delivery.nextMilestone, dueDate: project.delivery.dueDate, notes: project.delivery.notes } : { ownerId: lead!.owner?.id, offerId: lead!.offer?.id, priority: lead!.priority, temperature: lead!.temperature, probability: lead!.probability, expectedCloseDate: lead!.expectedCloseDate, outreachAngle: lead!.outreachAngle, fitScore: lead!.company.fitScore, qualificationNote: lead!.company.scaleNote },
  };
}
...
async function validate(actor: CurrentMember, input: GudDraftInput, saving = false) {
  ...
  if ((f.contactEmail || f.contactPhone || f.contactTitle || f.contactId) && !f.contact) throw new GudActionError("Include the contact's name when adding or updating contact details.");
...
export async function stageDraft(actor: CurrentMember, input: unknown, revision?: { id: string; version: number; appendTasks?: boolean }) {
  ...
  return withDatabaseTransaction(async () => {
    ...
    const draft: GudDraft = { ...merged, id: prior?.id ?? crypto.randomUUID(), version: (prior?.version ?? 0) + 1, status: "draft", expiresAt: prior?.expiresAt ?? new Date(Date.now() + 86400000).toISOString(), baseline: prior?.baseline ?? (thought ? String(thought.version) : record ? baseline(record) : "new"), label: thought ? thoughtLabel(thought) : record?.project ? `${record.project.companyName} · ${record.project.title}` : record?.lead ? `${record.lead.company.name} · ${record.lead.title}` : merged.fields.title || `New ${data.kind}`, warnings };
...
async function saveOne(actor: CurrentMember, draft: GudDraft) {
  ...
  if (draft.kind === "lead") {
    ...
    if (!id) {
      ...
      const created = await createOpportunity(actor, { ...details, companyName: f.company!, title: f.title!, ...(f.contact ? { contact: { name: f.contact, email: f.contactEmail, phone: f.contactPhone, title: f.contactTitle } } : {}), ...(f.task && dueAt ? { nextAction: { title: f.task, dueAt } } : {}) });
      id = created.id;
    } else {
      ...
      if (f.contact) {
        ...
        await saveContact(actor, { opportunityId: id, contactId: f.contactId, name: f.contact, email: f.contactEmail, phone: f.contactPhone, title: f.contactTitle });
      }
      if (f.task && dueAt) await setNextAction(actor, { opportunityId: id, title: f.task, dueAt });
    }
    if (f.fitScore !== undefined || f.qualificationNote !== undefined) {
      const { lead } = await selected(actor, { kind: "lead", id });
      await updateCompany(actor, { companyId: lead!.company.id, fitScore: f.fitScore, scaleNote: f.qualificationNote });
    }
