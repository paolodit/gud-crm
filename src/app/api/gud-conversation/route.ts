...
import { saveApprovalSchema } from "@/lib/gud-actions/save-approval";
...
import { connectVoice, contextSchema, endConversation, executeConversationTool, recordUsage, safeConversationError, startConversation, textConversation } from "@/lib/gud-actions/conversation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (data.op === "load" || data.op === "start") {
      ...
      return Response.json({ session, drafts, references, adapter: env.GUD_VOICE_ADAPTER });
    }
    ...
    if (data.op === "voice") { const input = z.object({ sdp: z.string().min(10).max(30000), context: contextSchema, voice: z.enum(gudVoices).default("marin"), pace: z.enum(["quick", "relaxed"]).default("quick") }).strict().parse(data.input); return Response.json(await connectVoice(actor, sessionId, input.sdp, input.context, input.voice, input.pace)); }
    if (data.op === "tool") { const input = z.object({ name: z.string().max(80), arguments: z.unknown(), context: contextSchema, approval: saveApprovalSchema.optional() }).strict().parse(data.input); return Response.json(await executeConversationTool(actor, sessionId, input.name, input.arguments, input.context.timezone, input.approval)); }
