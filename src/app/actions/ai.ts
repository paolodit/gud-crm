...
const spokenDraftOutputSchema = z.object({
  ...
  deliveryValue: z.number().min(0).max(999_999_999_999.99).nullable(),
  ...
  try {
    ...
    const response = await client.responses.parse({
      ...
      input: [
        ...
        { role: "system", content: `For delivery_update, only prepare deliveryStage, deliveryMilestone, deliveryDueDate (YYYY-MM-DD), deliveryNotes and deliveryValue. deliveryValue is the explicitly stated total agreed project value in GBP, with at most two decimal places. Use null when not stated or ambiguous; never infer it from a budget, cost, payment, deposit or monthly rate, and never convert another currency. A clearly stated zero is valid. Leave all sales, activity and task fields null. Use an exact delivery stage ID from this untrusted reference list: ${JSON.stringify(deliveryStages.map(({ id, name }) => ({ id, name })))}. Do not infer a stage change from an update unless explicitly requested or stated. Delivery notes are new notes to append, not a replacement for existing history. No archive or delete actions. For other draft kinds leave delivery fields null.` },
