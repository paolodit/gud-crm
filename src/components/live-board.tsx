...
function DeliveryEditor({ project, snapshot, stages, creating, pending, message, voiceAiConfigured, onClose, onSave }: { project: LiveProject; snapshot: BoardSnapshot; stages: DeliveryStage[]; creating: boolean; pending: boolean; message: string; voiceAiConfigured: boolean; onClose: () => void; onSave: (project: LiveProject) => Promise<void> }) {
  ...
  return <div className="dialog-backdrop" data-closing={closing} onClick={(e) => { if (e.target === e.currentTarget) requestClose(); }}><section ref={ref} className="dialog-card" role="dialog" aria-modal="true" aria-labelledby="delivery-title"><header className="dialog-header"><div><span className="eyebrow">Live project</span><h2 id="delivery-title">{creating ? "Add a project" : project.companyName}</h2><p>{creating ? "Already working together? Start here, without a sales cycle." : project.title}</p></div><button className="icon-button" type="button" disabled={pending} onClick={requestClose} aria-label="Close project"><X size={18} /></button></header><form className="dialog-form" onSubmit={submit} onChange={() => setHasManualEdits(true)}><div className="delivery-voice-toolbar">{!creating ? <WorkspaceVoiceButton target={{ kind: project.source === "direct" ? "direct" : "delivery", id: project.id }} disabled={pending || hasManualEdits || Boolean(project.delivery.archivedAt)} beforeOpen={onClose} /> : <VoiceFillButton kind="delivery_update" creatingProject={creating} aiConfigured={voiceAiConfigured} onDraft={(draft) => { const result = mergeDeliveryDraft(delivery, draft, stages); setDelivery(result.delivery); if (result.count && draft.deliveryValue != null) setValue(String(result.delivery.projectValue)); return result.count; }} />}{hasManualEdits && !creating ? <small className="muted">Save or close your manual edits before starting a combined voice update.</small> : null}</div><div className="form-grid">
    {project.source === "direct" ? <><label className="field-label">Project name<input className="field" name="title" required maxLength={200} defaultValue={project.title} /></label><label className="field-label">Organisation / client<input className="field" name="companyName" required maxLength={200} defaultValue={project.companyName} /></label><label className="field-label">Owner<select className="field-select" aria-label="Owner" name="ownerId" defaultValue={project.ownerId ?? ""}><option value="">Unassigned</option>{snapshot.users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label className="field-label">Offer<select className="field-select" name="offerId" defaultValue={project.offerId ?? ""}><option value="">No offer</option>{snapshot.offers.map((offer) => <option key={offer.id} value={offer.id}>{offer.name}</option>)}</select></label></> : null}
