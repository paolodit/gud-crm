...
export function ThoughtsBoard({ initial }: { initial: BoardData }) {
  ...
  return <div className="thoughts-page">
    <header className="thoughts-header"><div><span className="thoughts-private"><LockKeyhole size={13} /> Only you</span><h1>Thoughts</h1><p>A little room for whatever is on your mind.</p></div><button type="button" className="btn btn-secondary" onClick={() => setPanel("all")}><Library size={16} />Explorations <span>{explorations.length}</span></button></header>
    ...
    <div className="thoughts-workspace"><div ref={viewport} className="thoughts-viewport" {...pan}>
      <DndContext id={id} sensors={sensors} onDragEnd={({ active, delta }) => {
        ...
        {shown.map((note) => <ThoughtNote key={note.id} note={note} disabled={busy} explorationCount={explorations.filter((item) => item.thoughtId === note.id).length} onOpen={() => setEditor(note)} onExplore={() => setPanel(note.id)} onCheck={(taskId) => void save({ ...contentOf(note), checklist: note.checklist.map((item) => item.id === taskId ? { ...item, done: !item.done } : item) }, note)} />)}
      </div></DndContext>
    </div>{panel ? <ExplorationPanel selection={panel} notes={notes} explorations={explorations} aiEnabled={initial.aiEnabled} onSelect={setPanel} onClose={() => setPanel(null)} onCreated={(document) => setExplorations((items) => [document, ...items])} /> : null}</div>
...
function ThoughtNote({ note, disabled, explorationCount, onOpen, onExplore, onCheck }: { note: Thought; disabled: boolean; explorationCount: number; onOpen: () => void; onExplore: () => void; onCheck: (id: string) => void }) {
  ...
  return <article ref={setNodeRef} className="thought-note" data-colour={note.colour} data-dragging={isDragging} data-thought-id={note.id} style={{ left: note.x, top: note.y, transform: transform ? `translate3d(${transform.x}px, ${transform.y}px, 0)` : undefined, zIndex: isDragging ? 100 : undefined }}>
    <div className="thought-note-top"><span>{note.category || "Thought"}</span><button type="button" className="thought-grip" disabled={disabled} aria-label={`Move thought ${thoughtLabel(note)}`} {...attributes} {...listeners}><Grip size={16} /></button></div>
    <button type="button" className="thought-note-open" aria-label={`Edit thought ${thoughtLabel(note)}`} onClick={onOpen}>{note.title ? <h2>{note.title}</h2> : null}<p>{note.body}</p></button>
...
function ThoughtEditor({ initial, existing, categories, notes, busy, error, onClose, onSave }: { initial: ThoughtContent; existing?: Thought; categories: string[]; notes: Thought[]; busy: boolean; error: string; onClose: () => void; onSave: (content: ThoughtContent, archive: boolean) => Promise<void> }) {
  const [value, setValue] = useState(initial);
  const dirty = JSON.stringify(initial) !== JSON.stringify(value);
  const close = () => { if (!busy && (!dirty || window.confirm("Discard the unsaved changes to this thought?"))) onClose(); };
  const focus = useDialogFocus(close);
  const categoryId = useId();
  return <div className="dialog-backdrop thoughts-modal"><section ref={focus} className="dialog-card thought-editor" role="dialog" aria-modal="true" aria-labelledby="thought-editor-title" tabIndex={-1}><header><h2 id="thought-editor-title">{existing ? "Your thought" : "New thought"}</h2><button type="button" className="icon-button" aria-label="Close thought editor" onClick={close} disabled={busy}><X size={18} /></button></header><form onSubmit={(event) => { event.preventDefault(); void onSave(value, existing?.archived ?? false); }}>
    <label className="field-label">Title <small>optional</small><input autoComplete="off" maxLength={160} value={value.title} onChange={(event) => setValue({ ...value, title: event.target.value })} disabled={busy} /></label>
    <label className="field-label">Thought<textarea data-autofocus rows={7} maxLength={20000} placeholder="No need to make it neat. Bullets, questions, longer notes…" value={value.body} onChange={(event) => setValue({ ...value, body: event.target.value })} disabled={busy} /></label>
    ...
    {error ? <p role="alert" className="thoughts-error">{error}</p> : null}<footer>{existing ? <button type="button" className="btn btn-quiet" disabled={busy} onClick={() => void onSave(value, !existing.archived)}><Archive size={14} />{existing.archived ? "Restore thought" : "Archive thought"}</button> : <span />}<button type="submit" className="btn btn-primary" disabled={busy || !(value.title.trim() || value.body.trim() || value.checklist.length)}>{busy ? "Saving…" : "Save thought"}</button></footer>
  </form></section></div>;
}
...
function ExplorationPanel({ selection, notes, explorations, aiEnabled, onSelect, onClose, onCreated }: { selection: string; notes: Thought[]; explorations: ThoughtExploration[]; aiEnabled: boolean; onSelect: (id: string) => void; onClose: () => void; onCreated: (document: ThoughtExploration) => void }) {
  ...
  async function explore(mode: "ai" | "outline") {
    ...
    try {
      const result = await exploreThoughtAction({ id: note.id, mode, research: mode === "ai" && research });
      if (!result.ok) { setError(result.error); return; }
  ...
  return <aside ref={section} tabIndex={-1} className="thought-exploration-panel" data-wide={wide} aria-label="Private explorations" style={{ "--thought-panel-width": `${panelWidth}px` } as CSSProperties} onKeyDown={(event) => { if (event.key === "Escape" && !pending) onClose(); }}>
    ...
    {note ? <section className="thought-explore-controls"><h3>A little deeper, without making the note bigger.</h3><p>Each exploration is a new document. The original thought stays as it is.</p>{aiEnabled ? <><p className="thought-ai-notice">Generate sends only this thought and its checklist to the configured OpenAI provider. Provider retention policies apply. No other Thoughts are included.</p><label className="thought-research"><input type="checkbox" checked={research} onChange={(event) => setResearch(event.target.checked)} disabled={pending} />Include web research</label>{research ? <small>Search queries may be shared with search providers. Avoid this for sensitive ideas.</small> : null}<button type="button" className="btn btn-primary" disabled={pending} onClick={() => void explore("ai")}><Sparkles size={15} />{pending ? "Exploring…" : "Generate exploration"}</button></> : <p>AI exploration is not configured. You can still create an offline thinking outline.</p>}<button type="button" className="btn btn-quiet" disabled={pending} onClick={() => void explore("outline")}>Create thinking outline · no AI</button></section> : null}
...
function ExplorationText({ document }: { document: ThoughtExploration }) {
  ...
  return <div className="thought-document-text">{lines.map((line, lineIndex) => {
    ...
    for (const source of sources) { const localStart = source.start - start, localEnd = source.end - start; if (localStart < cursor) continue; parts.push(line.slice(cursor, localStart)); parts.push(<a key={`${source.url}:${source.start}`} href={source.url} target="_blank" rel="noopener noreferrer">[{source.title || "Source"}]</a>); cursor = localEnd; }
