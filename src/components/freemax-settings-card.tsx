...
export function FreeMaxSettingsCard({ status, canManage }: { status: FreeMaxStatus; canManage: boolean }) {
  ...
  return (
    <article className="surface settings-card settings-card-wide enrichment-settings-card enrichment-settings-card-live" id="enrichment">
      <div className="settings-icon settings-icon-enrichment"><Sparkles /></div>
      <div><h2>FreeMax enrichment</h2><p>Find a named contact’s work email while protecting free allowances</p></div>
      ...
      <p className="freemax-explainer"><ShieldCheck size={15} /><span>Keys are encrypted with this deployment’s secret and are never sent back to the browser. Successful lookups are counted here; provider dashboards remain authoritative.</span></p>
      {open && canManage ? (
