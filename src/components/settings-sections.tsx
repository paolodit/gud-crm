...
export function SettingsSections({ children }: { children: ReactNode }) {
  ...
  const [active, setActive] = useState(tabs.some(([id]) => id === requestedTab) ? requestedTab! : "workspace");
  useEffect(() => {
    function sync() {
      const url = new URL(window.location.href);
      const requested = url.searchParams.get("tab") ?? (url.hash.includes("ai") || url.hash === "#freemax" || url.hash === "#email-finding" ? "ai" : url.hash === "#import" ? "system" : "workspace");
  ...
  useEffect(() => {
    if (active === "ai" && (searchParams.get("section") === "email-finding" || ["#email-finding", "#freemax"].includes(window.location.hash))) {
      document.getElementById("email-finding")?.scrollIntoView({ block: "start" });
    }
  }, [active, searchParams]);
  function select(id: string) { setActive(id); const url = new URL(window.location.href); url.searchParams.set("tab", id); url.searchParams.delete("section"); url.hash = ""; window.history.replaceState(null, "", url); }
