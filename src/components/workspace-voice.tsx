...
export function WorkspaceVoiceProvider({ memberKey, voicePreferences = null, conversationEnabled = false, children }: { memberKey: string; voicePreferences?: string | null; conversationEnabled?: boolean; children: React.ReactNode }) {
  ...
  return <VoiceContext.Provider value={{ open: show }}>{children}
    ...
    <button className="workspace-voice-launch" data-voice-state={conversationVoice} type="button" onClick={() => show()} aria-label="Talk to GUD" aria-describedby="gud-launch-voice-status" title="Talk to GUD (Ctrl/⌘ + Shift + Space)"><Mic size={19} /><span>{conversationVoice === "paused" ? "Resume GUD" : conversationVoice === "speaking" ? "GUD is hearing you" : conversationVoice === "listening" ? "GUD · mic on" : "Talk to GUD"}</span><span id="gud-launch-voice-status" className="sr-only">{conversationVoice === "speaking" ? "Hearing you" : conversationVoice === "listening" ? "Microphone on" : conversationVoice === "paused" ? "Microphone paused" : "Microphone off"}</span>{session.transcript.trim() ? <i aria-label="Draft saved" /> : null}</button>
...
function WorkspaceVoiceDialog({ preferredTarget, pageScope, session, onSession, onClose, onApplied }: { preferredTarget: VoiceTarget | null; pageScope: VoiceScope; session: VoiceSession; onSession: (session: VoiceSession) => void; onClose: () => void; onApplied: (receipt: VoiceReceipt) => void }) {
  ...
  const [resumedTarget] = useState<VoiceTarget | null>(session.transcript.trim() ? session.target : null);
  const requestedScope = preferredTarget ? voiceScopeForTarget(preferredTarget) : pageScope;
  const [scope, setScope] = useState<VoiceScope>(() => session.transcript.trim() && session.target ? voiceScopeForTarget(session.target) : requestedScope);
  ...
  const speech = useSpeechCapture(setTranscript, !session.pending, session.transcript);
  useEffect(() => {
  ...
  const matchingChoices = voiceChoicesForScope(choices, scope, search);
  const differentDraft = Boolean(transcript.trim() && resumedTarget && selected && choiceKey(selected.target) === choiceKey(resumedTarget) && (
    (requestedScope !== "all" && voiceScopeForTarget(selected.target) !== requestedScope)
    || (preferredTarget && choiceKey(selected.target) !== choiceKey(preferredTarget))
  ));
  ...
  return <div className="dialog-backdrop workspace-voice-backdrop"><section ref={ref} className="dialog-card workspace-voice-dialog" role="dialog" aria-modal="true" aria-labelledby="workspace-voice-title">
    ...
    {review ? <VoiceReview review={review} timezone={timezone} pending={busy} error={error} onBack={editWords} onApply={apply} /> : <>
      <div className="workspace-voice-body">
        <div className="voice-target"><span className="voice-step-label">1 · {scope === "delivery" ? "Choose the live project" : scope === "sales" ? "Choose the pipeline record" : "Choose the record"}</span>{loading ? <p role="status"><LoaderCircle size={15} className="spin" />Loading your records…</p> : selected ? <div className="voice-selected"><div><small>{kindName(selected.target)}</small><strong>{selected.companyName}</strong><span>{selected.title}</span></div><button type="button" className="btn btn-quiet" disabled={busy || !captureIdle} onClick={() => setTarget(null)}>Change</button></div> : <><div className="voice-scope-toggle view-toggle" role="group" aria-label="Voice record scope">{(["sales", "delivery", "all"] as const).map((value) => <button key={value} type="button" aria-pressed={scope === value} disabled={busy || !captureIdle} onClick={() => { setScope(value); setSearch(""); }}>{voiceScopeLabel(value)}</button>)}</div><label className="search-box"><Search size={16} /><input aria-label="Find a voice record" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={scope === "delivery" ? "Find a live project" : scope === "sales" ? "Find a pipeline record" : "Find a client or project"} /></label><div className="voice-record-list">{matchingChoices.slice(0, 30).map((choice) => <button type="button" key={choiceKey(choice.target)} onClick={() => setTarget(choice.target)}><span><strong>{choice.companyName}</strong><small>{choice.title}</small></span><em>{kindName(choice.target)}</em></button>)}{!matchingChoices.length ? <p>{search.trim() ? "No matching records. Try another name or change the scope." : scope === "delivery" ? "No live projects yet. Add one on the Live projects board." : scope === "sales" ? "No pipeline records yet. Add an opportunity first." : "Add a sales record or live project first."}</p> : null}</div><small>Choose explicitly when a client has more than one record.</small></>}{differentDraft ? <p className="voice-context-help" role="status">Resuming your saved {kindName(selected!.target).toLowerCase()} draft for {selected!.companyName}. Discard words to start the update you just opened; your draft has not been moved to another record.</p> : null}</div>
