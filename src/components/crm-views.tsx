...
export function PlaybookWorkspace({ offers, assetsByOffer, canManageAssets }: { offers: OfferSummary[]; assetsByOffer: Record<string, SalesAssetSummary[]>; canManageAssets: boolean }) {
  ...
  const categories = ["All", ...new Set(playbookItems.map((item) => item.category))];
  ...
  return (
    <WorkspaceFrame title="Playbook" subtitle="A practical route from research to a useful conversation">
      <section className="playbook-hero"><span className="playbook-hero-icon"><BookOpen size={24} /></span><div><span className="eyebrow">{selectedOffer?.name ?? "Outreach"} system</span><h2>Be useful before asking for attention.</h2><p>{selectedOffer?.positioning || "Start with a real observation, choose the least-friction route, record the signal, and make the next step proportionate."}</p>{offers.length > 1 ? <label className="playbook-offer-select">Playbook for<select value={selectedOfferId} onChange={(event) => { setSelectedOfferId(event.target.value); setSelectedAssetId(null); }}>{offers.map((offer) => <option key={offer.id} value={offer.id}>{offer.name}</option>)}</select></label> : null}</div><aside><small>Core message shape</small><strong>Observation → relevance → proof → small ask</strong><span>{selectedOffer?.idealCustomer || "Four parts. Usually under 90 words."}</span></aside></section>

      <section className="playbook-map surface"><header><div><span className="eyebrow">The operating loop</span><h2>Every touch should move the knowledge forward</h2></div><span className="playbook-map-rule"><Timer size={15} />No response is data—not permission to repeat the same message.</span></header><div className="playbook-flow">{playbookFlow.map((step, index) => { const Icon = step.icon; return <div className="playbook-flow-step" data-tone={step.tone} key={step.title}><span><Icon size={17} /></span><div><strong>{step.title}</strong><small>{step.detail}</small></div>{index < playbookFlow.length - 1 ? <ArrowRight className="playbook-flow-arrow" size={16} /> : null}</div>; })}</div></section>

      <section className="cadence-board"><div><span className="eyebrow">A sensible first sequence</span><h2>Vary the route, not the pressure</h2><p>Stop or adapt when the evidence changes. This is a guide, not an automation.</p></div><ol><li><b>Day 1</b><span><strong>LinkedIn or email</strong><small>Specific observation + one small question</small></span></li><li><b>Day 4–6</b><span><strong>Use a second channel</strong><small>Add new value; do not write “just following up”</small></span></li><li><b>Day 10–14</b><span><strong>Close the loop</strong><small>Ask whether to pause, redirect or return on a trigger</small></span></li></ol></section>

      <section className="asset-kit surface"><header><div><span className="eyebrow">Sales asset kit{selectedOffer ? ` · ${selectedOffer.name}` : ""}</span><h2>Choose the right proof for the moment</h2><p>Browse the six core assets. Open one only when you need its link, status or production note.</p></div><span className="asset-kit-progress">{assetList.filter((asset) => asset.status === "ready").length} / {assetList.length} ready</span></header><div className="asset-browser">{salesAssetDefinitions.map((definition) => { const asset = assetList.find((item) => item.id === definition.id); if (!asset) return null; const Icon = definition.icon; return <button className="asset-tile" data-status={asset.status} aria-pressed={selectedAssetId === asset.id} type="button" key={definition.id} onClick={() => setSelectedAssetId((current) => current === asset.id ? null : asset.id)}><span className="asset-tile-icon"><Icon size={20} /></span><span><strong>{definition.title}</strong><small>{definition.purpose}</small></span><i>{assetStatusLabel(asset.status)}</i><ChevronDown size={17} /></button>; })}</div>{selectedAssetId ? (() => { const definition = salesAssetDefinitions.find((item) => item.id === selectedAssetId); const asset = assetList.find((item) => item.id === selectedAssetId); return definition && asset && selectedOffer ? <SalesAssetEditor definition={definition} asset={asset} offerId={selectedOffer.id} canManage={canManageAssets} onClose={() => setSelectedAssetId(null)} onSaved={(saved) => setAssetLists((items) => ({ ...items, [selectedOffer.id]: (items[selectedOffer.id] ?? []).map((item) => item.id === saved.id ? saved : item) }))} /> : null; })() : null}</section>

      <div className="playbook-nav"><span>Jump to</span><div className="segmented" role="tablist" aria-label="Playbook category">{categories.map((item) => <button role="tab" aria-selected={category === item} key={item} onClick={() => setCategory(item)}>{item}</button>)}</div></div>
      <section className="playbook-grid">{playbookItems.filter((item) => category === "All" || item.category === category).map((item) => { const body = adaptPlaybookBody(item.body, selectedOffer); return <details className="playbook-card" data-category={item.category.toLowerCase()} key={item.id}><summary><span><span className="eyebrow">{item.category}</span><strong>{item.title}</strong><small>{item.description}</small></span><ChevronDown size={19} /></summary><div className="playbook-card-content"><div className="playbook-card-meta"><div><small>Use when</small><span>{item.when}</span></div><div><small>Guardrail</small><span>{item.guardrail}</span></div></div><div className="playbook-copy"><span>Adapt this</span><p>{body}</p></div><button className="btn btn-quiet" type="button" onClick={() => copy(item.id, body)}>{copied === item.id ? <Check size={15} /> : <Copy size={15} />}{copied === item.id ? "Copied" : "Copy pattern"}</button></div></details>; })}</section>
      <p className="playbook-note"><ShieldCheck size={16} /> Patterns are starting points. Replace every bracketed field, use approved proof only, read it aloud, and never send personal data to an AI tool without approval.</p>
    </WorkspaceFrame>
  );
}
...
export function SettingsDashboard({ snapshot, runtime, importStatus, currentMemberId, currentRole }: { snapshot: BoardSnapshot; runtime: RuntimeStatus; importStatus: ImportStatus; currentMemberId: string; currentRole: "admin" | "manager" | "member" }) {
  ...
  const [enrichmentSetupOpen, setEnrichmentSetupOpen] = useState(false);
  const [enrichmentConfigCopied, setEnrichmentConfigCopied] = useState(false);
  ...
  async function copyEnrichmentConfiguration() {
    await navigator.clipboard.writeText("HUNTER_API_KEY=your-server-side-key\nVOILA_NORBERT_API_KEY=your-server-side-key\nHUNTER_FREE_MONTHLY_LIMIT=50\nNORBERT_FREE_LIFETIME_LIMIT=50");
    setEnrichmentConfigCopied(true);
    window.setTimeout(() => setEnrichmentConfigCopied(false), 1_800);
  }
  ...
  return (
    <WorkspaceFrame title="Settings" subtitle="Workspace shape, data readiness and operational guardrails">
      ...
      <section className="settings-grid">
        ...
        <article className="surface settings-card settings-card-wide enrichment-settings-card" id="enrichment"><div className="settings-icon settings-icon-enrichment"><Sparkles /></div><div><h2>FreeMax enrichment</h2><p>Use free contact-finding allowances carefully, with a visible cap</p></div>{currentRole === "admin" ? <button className="btn btn-primary settings-card-action" type="button" onClick={() => setEnrichmentSetupOpen((value) => !value)}><KeyRound size={14} />{enrichmentSetupOpen ? "Close setup" : "Connect providers"}</button> : null}<div className="enrichment-provider-grid"><div data-ready={runtime.freeMaxStatus.hunter.configured}><span><strong>Hunter</strong><small>First choice · monthly allowance</small></span><b>{runtime.freeMaxStatus.hunter.configured ? `${runtime.freeMaxStatus.hunter.used}/${runtime.freeMaxStatus.hunter.limit} used` : "Not connected"}</b></div><div data-ready={runtime.freeMaxStatus.norbert.configured}><span><strong>Voila Norbert</strong><small>Fallback only · starter allowance</small></span><b>{runtime.freeMaxStatus.norbert.configured ? `${runtime.freeMaxStatus.norbert.used}/${runtime.freeMaxStatus.norbert.limit} used` : "Not connected"}</b></div></div><p className="freemax-explainer"><ShieldCheck size={15} /><span><strong>How FreeMax works:</strong> GUD tries Hunter first, then Norbert only when needed. It stops at the safety caps and never silently enters paid usage. Provider dashboards remain authoritative.</span></p>{enrichmentSetupOpen && currentRole === "admin" ? <div className="ai-setup-panel enrichment-setup-panel"><div className="ai-setup-step"><b>1</b><span><strong>Create the free provider accounts</strong><small>Start with Hunter. Add Norbert as a fallback when useful.</small></span><div className="button-row"><a className="btn btn-quiet" href="https://hunter.io/users/sign_up" target="_blank" rel="noreferrer">Hunter <ExternalLink size={13} /></a><a className="btn btn-quiet" href="https://www.voilanorbert.com/" target="_blank" rel="noreferrer">Norbert <ExternalLink size={13} /></a></div></div><div className="ai-setup-step"><b>2</b><span><strong>Add the keys to the server</strong><small>In CapRover, use App Configs → Environment Variables, then Save & Restart.</small></span></div><code className="ai-env-block">HUNTER_API_KEY=your-server-side-key{"\n"}VOILA_NORBERT_API_KEY=your-server-side-key{"\n"}HUNTER_FREE_MONTHLY_LIMIT=50{"\n"}NORBERT_FREE_LIFETIME_LIMIT=50</code><div className="ai-setup-actions"><button className="btn btn-quiet" type="button" onClick={copyEnrichmentConfiguration}>{enrichmentConfigCopied ? <Check size={14} /> : <Copy size={14} />}{enrichmentConfigCopied ? "Copied" : "Copy configuration"}</button><span><ShieldCheck size={14} />Keys remain server-side. GUD records successful lookups so the team can see its own usage.</span></div></div> : null}</article>
