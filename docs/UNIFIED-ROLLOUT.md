...
- Migrations 0000–0010 were historically applied from Windows CRLF tarballs on all three instances. Their original CRLF bytes are now retained in Git with explicit `-text` attributes and pinned-hash regression tests. No database ledger is rewritten, and the exact-hash guard remains unchanged.
